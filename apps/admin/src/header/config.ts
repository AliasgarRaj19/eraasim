...
export const headerConfigSchema = z.object({
  ...
  defaults: z.object({ homeVisible: z.boolean(), blogVisible: z.boolean(), aboutVisible: z.boolean(), contactVisible: z.boolean(), careersVisible: z.boolean(), order: z.array(z.enum(["home", "blog", "about", "contact", "careers"])).length(5).refine((v) => new Set(v).size === 5) }),
...
export const defaultHeaderConfig: HeaderConfig = { branding: { mode: "text", line1: "Eraasim", line2: "Stories & journeys", logoPath: "" }, defaults: { homeVisible: true, blogVisible: true, aboutVisible: true, contactVisible: true, careersVisible: false, order: ["home", "blog", "about", "contact", "careers"] }, manualItems: [], announcement: { visible: false, message: "" } };
export function normalizeHeaderConfig(value: unknown) { if (!value || typeof value !== "object") return value; const current=value as Record<string,unknown>,defaults=current.defaults as Record<string,unknown>|undefined;if(!defaults)return value;let order=Array.isArray(defaults.order)?defaults.order:[];if(!order.includes("about"))order=[...order.filter(key=>key!=="contact"),"about",...order.filter(key=>key==="contact")];if(!order.includes("careers"))order=[...order,"careers"];return{...current,defaults:{...defaults,aboutVisible:typeof defaults.aboutVisible==="boolean"?defaults.aboutVisible:true,careersVisible:typeof defaults.careersVisible==="boolean"?defaults.careersVisible:false,order}}; }
export function parseHeaderConfig(value: unknown) { return headerConfigSchema.safeParse(normalizeHeaderConfig(value)); }
export function parseHeaderForm(formData: FormData) { let manualItems: unknown = []; let order: unknown = []; try { manualItems = JSON.parse(String(formData.get("manualItems") ?? "[]")); order = JSON.parse(String(formData.get("defaultOrder") ?? "[]")); } catch { /* validation reports malformed data */ } return headerConfigSchema.safeParse({ branding: { mode: String(formData.get("brandingMode") ?? ""), line1: String(formData.get("line1") ?? ""), line2: String(formData.get("line2") ?? ""), logoPath: String(formData.get("logoPath") ?? "") }, defaults: { homeVisible: formData.get("homeVisible") === "on", blogVisible: formData.get("blogVisible") === "on", aboutVisible: formData.get("aboutVisible") === "on", contactVisible: formData.get("contactVisible") === "on", careersVisible: formData.get("careersVisible") === "on", order }, manualItems, announcement: { visible: formData.get("announcementVisible") === "on", message: String(formData.get("announcementMessage") ?? "") } }); }
