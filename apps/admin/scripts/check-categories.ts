...
parentForm.set("seoTitle", "Gujarati Food Stories");
parentForm.set("seoDescription", "Published Gujarati food stories.");
parentForm.set("featuredPostId", "");
const parsedParent = parseCategoryInput(parentForm);
assert(parsedParent.success && parsedParent.data.slug === "gujarati-food");
assert(parsedParent.success && parsedParent.data.seoTitle === "Gujarati Food Stories" && parsedParent.data.featuredPostId === null);
...
assert(actionSource.includes("INVALID_FEATURED_POST") && actionSource.includes('eq(posts.categoryId,current.id)') && actionSource.includes('eq(posts.status,"published")') && actionSource.includes("isNull(posts.deletedAt)"));
const editorialMigration=readFileSync(new URL("../drizzle/0009_category_page_editorial.sql",import.meta.url),"utf8");assert(editorialMigration.includes('ADD COLUMN "seo_title" text')&&editorialMigration.includes('ADD COLUMN "seo_description" text')&&editorialMigration.includes('ADD COLUMN "featured_post_id" uuid')&&editorialMigration.includes("ON DELETE set null"));
