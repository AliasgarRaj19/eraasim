...
import { eligibleGenericPages } from "@/src/generic-pages/eligible";
export default async function HeaderPage({ searchParams }: { searchParams: Promise<{ result?: string }> }) { const { authorization } = await requireRouteAccess("/header"); const [[row], pageOptions, query] = await Promise.all([db.select({ draft: headerConfigurations.draft, draftVersion: headerConfigurations.draftVersion }).from(headerConfigurations).where(eq(headerConfigurations.id, "header")).limit(1), eligibleGenericPages(), searchParams]); const parsed = parseHeaderConfig(row?.draft); return <HeaderForm config={parsed.success ? parsed.data : defaultHeaderConfig} pageOptions={pageOptions} version={row?.draftVersion ?? 0} canEdit={canUsePermission(authorization, "header.edit")} canPublish={canUsePermission(authorization, "header.publish")} result={query.result} />; }
