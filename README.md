...
Contact enquiries are stored in PostgreSQL before notification is attempted. Production delivery uses the Resend transactional email API directly and requires runtime-only `RESEND_API_KEY` and `CONTACT_EMAIL_FROM` values on `eraasim-web`. The sender address/domain must be verified in Resend; the recipient remains exclusively in the Published Contact Us CMS. Builds do not require these values. If either value is absent or the provider rejects delivery, the enquiry remains stored with **Not sent** status and a sanitized error is written to the public-service log.
