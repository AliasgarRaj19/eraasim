...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <article className="article"><header className="article-header">{post.categoryName && post.categorySlug ? <Link className="category-chip" href={`/categories/${post.categorySlug}`}>{post.categoryName}</Link> : null}<h1>{post.title}</h1><p className="article-summary">{post.shortDescription}</p><div className="post-meta"><span>By {post.authorName}</span><time dateTime={post.publishedAt.toISOString()}>{formatter.format(post.publishedAt)}</time></div></header>{image ? <Image className="article-featured" src={image} alt="" width={1200} height={675} priority unoptimized /> : null}<TiptapContent content={post.content} /><div className="future-discussion-slot" aria-hidden="true" /></article>;
}
