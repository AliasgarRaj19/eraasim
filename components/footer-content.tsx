...
export function FooterContent({ config, categories, pages, subscriberSettings }: { config: FooterConfig; categories: FooterCategory[]; pages: FooterPage[]; subscriberSettings?: PublicSubscriberSettings }) {
  ...
  return <footer className="public-footer">
    ...
    <div className="footer-brand-lines"><div>{config.content.heading ? <strong>{config.content.heading}</strong> : null}{config.content.description ? <p>{config.content.description}</p> : null}</div>{subscriberSettings?.enabled ? <FooterSubscribeForm buttonLabel={subscriberSettings.buttonLabel} /> : null}</div>
